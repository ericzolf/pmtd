...
public class IntegerProvider implements INumberProvider {
	...
	// TODO decide if this function fits better here or directly in the IPrefs implementations (with an abstract class)
	public String getPrefsString() {
		...
		case PLUS:
			...
			break;
		case MINUS:
			if (prefs.isMinusNegativeAllowed()) {
				buf.append('∓'); // U+2213 (8723) 	∓ 	MINUS-OR-PLUS SIGN
			} else {
		...
		case DIVIDE:
			if (prefs.isDivideRestAllowed()) {
				buf.append('∤'); // U+2224 (8740) 	∤ 	DOES NOT DIVIDE
			} else {
				buf.append('∣'); // U+2223 (8739) 	∣ 	DIVIDES
			}
			if (prefs.isDivideIntegers()) {
